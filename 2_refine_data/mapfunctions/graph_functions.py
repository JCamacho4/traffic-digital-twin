...
def init_graph_bbox(north, south, east, west, osm_ways_to_delete=None):
    ...
    for u, v, edge_data in graph.edges(data=True):
        edge_data["dates"] = {}
        edge_data["current_speed"] = None
        edge_data["api_data"] = False
        edge_data["traffic_level"] = None
...
def __setup_maxspeed(graph,
    ...
    for u, v, data in graph.edges(data=True):
        ...
        if 'maxspeed' not in data:
            ...
            elif data['highway'] == 'primary_link':
                data['maxspeed'] = primary_link
            elif data['highway'] == 'secondary_link':
                data['maxspeed'] = secondary_link
            elif data['highway'] == 'living_street':
                data['maxspeed'] = living_street
            else:
                data['maxspeed'] = None
...
def add_traffic_level_from_folder(graph, folder, precision=6, save_each_graph_mongo=False):
