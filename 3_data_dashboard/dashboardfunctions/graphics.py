...
def create_horizontal_bars_by_name_graph(data, selected_category):
    ...
    # Sort the data based on the selected category in descending order
    sorted_data = sorted(data, key=lambda x: x[selected_category], reverse=False)
...
def create_vertical_bars_by_hours_graph(data, selected_category):
    ...
    # Sort data by time if necessary, assuming '_id' represents time
    sorted_data = sorted(data, key=lambda x: x['_id'])

    # Extract x and y values
    x_values = [item['_id'] for item in sorted_data]
    y_values = [item[selected_category] for item in sorted_data]
    ...
    # Update layout with autoscaling and more space for x-axis labels
    fig.update_layout(
...
def create_horizontal_bars_by_weekday_graph(data, selected_category):
    if len(data) == 0:
        return go.Figure()  # Return an empty figure initially
    ...
    # Sort data according to the weekday order
    sorted_data = sorted(sorted_data, key=lambda x: weekday_order.index(x['_id']))
