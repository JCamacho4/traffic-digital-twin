...
def load_from_osmnx_graph(g, date=""):
    """
    ...
    def get_coords_list(edge):
        if "geometry" in edge[2].keys():
            ...
            return [[float(g.nodes(data=True)[edge[0]]["y"]), float(g.nodes(data=True)[edge[0]]["x"])],
                    [float(g.nodes(data=True)[edge[1]]["y"]), float(g.nodes(data=True)[edge[1]]["x"])]]
    ...
    nodes_data = [
        {
            ...
            "data": {
                ...
                "street_count": node[1]["street_count"] if "street_count" in node[1].keys() else None,
                "x": node[1]["x"],
                "y": node[1]["y"],
                "traffic_light": node[1]["traffic_light"] if "traffic_light" in node[1].keys() else None,
            }
        }
    ...
    edges_data = [
        {
            ...
            "data": {
                ...
                "source_osmid": edge[0],
                "target_osmid": edge[1],
                "traffic_level": edge[2]['traffic_level'] if 'traffic_level' in edge[2].keys() and edge[2]['traffic_level'] != "None" else None,
                "current_speed": edge[2]['current_speed'] if 'current_speed' in edge[2].keys() and edge[2]['current_speed'] != "None" else None,
                "api_data": edge[2]['api_data'] if 'api_data' in edge[2].keys() else None,
                "bearing": edge[2]['bearing'] if 'bearing' in edge[2].keys() else None,
            }
        }
...
def load_from_sumo_network(filen, sumolib_path="/usr/share/sumo/tools"):
    ...
    def covert_to_lonlat(coordinates):
        return g.convertXY2LonLat(
            *coordinates)  # convertXY2LonLat uses pyproj and seems to be the CPU-bounded bottleneck
